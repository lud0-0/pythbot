...
me_wordslist = ["AM", "I'M", "YOU'RE", "ME", "WHO AM I", "LOOK", "ME?"]
...
def time_since():
    return "**Time since startup:**\n`" + str(datetime.datetime.now() - uptime) + "`\n*Defined in H:M:S:MS*"
...
async def on_message(message):
    if message.content.upper().startswith('CLAP'):
        ...
        if message.author != bot.user:
            await bot.send_message(message.channel, time_since())
